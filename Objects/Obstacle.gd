...
signal deleted()
signal hit_with_projectile(hue)
...
var was_defeated_by_player:=false
...
func updateSpriteHue():
	...
	emit_signal("hit_with_projectile",hue)
...
func delete_self():
	if(was_defeated_by_player):
		emit_signal("deleted")
	else:
		emit_signal("out_of_bounds")
		print("Obstacle: Obstacle out of bounds!")
	self.queue_free()
...
func _on_Area2D_area_entered(area):
	if(enabled):
		if(area.is_in_group("Player")):
			...
			#print("Obstacle: collision with death plane!")
			emit_signal("out_of_bounds")
			self.queue_free()
